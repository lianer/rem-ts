(function (global, factory) {
  ...
  (global = global || self, global.rem = factory());
  ...
  function () {
    function Rem() {
      ...
      window.addEventListener('resize', function () {
        clearTimeout(timer);
        timer = window.setTimeout(_this.resize.bind(_this), _this.config.delay);
      });
    ...
    _createClass(Rem, [{
      ...
      value: function resize() {
        ...
        this.size = Math.max(this.config.min, Math.min(this.config.max, document.documentElement.clientWidth)) / this.config.num / scale;
